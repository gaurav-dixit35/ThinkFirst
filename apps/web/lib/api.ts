...
export type QuestionTracking = {version:string;current_question_event_id:string;event_questions:Record<string,string>;questions:{id:string;text:string;attempts:number;attempt_before_first_request:boolean|null;requests:Record<'answer'|'hint'|'check_thinking'|'exercise',number>;delivered:number;verification:Record<'checked'|'found_issue'|'not_checked',number>}[]};
export type SessionData = {question_tracking?:QuestionTracking|null;archived?:boolean;deletion_pending?:boolean;answer_style?:'concise'|'detailed';id: string; status: string; events: LogEvent[]; summary: Record<string, any>; experience?:'chat'|'guided'; mode?:ConversationMode; practice?:PracticeState;title?:string;custom_title?:boolean;overview?:ConversationOverview};

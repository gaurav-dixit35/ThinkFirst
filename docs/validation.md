...
- Complete PostgreSQL backend suite: **145 passed** on dedicated `thinkfirst_test`, including concurrency, runtime-role restrictions, erasure and event protection. An initial run on a differently named test database correctly tripped the runtime-role test's database-name guard; the final run used the required database without weakening that safeguard. Docker disconnected during an intervening run; restarting Docker and the existing container allowed the complete suite to pass. The main database volume was not recreated.
...
- API Dockerfile built successfully with an isolated BuildKit builder. A final network-disabled smoke step on the same image filesystem passed: UID 10001, no copied `.env` or configured provider credentials, startup through `apps.api.serve` on assigned `PORT=48765`, database initialization, health 200 and request reference. Docker Desktop's default image store still has a missing parent snapshot (`snapshots/82/fs`), so ordinary `docker run` of this freshly imported image could not extract its base layer. The isolated build-container smoke bypassed that host cache defect; it does not claim the default Docker cache is repaired. No global prune, factory reset or database-volume removal was performed. Temporary QA servers and the isolated builder were removed afterward.
...
Final local runtime: PostgreSQL `res-db-1` is healthy, the normal Clerk-enabled API runs on `127.0.0.1:8000`, and the website runs on `http://localhost:3000`. Health and signup HTTP checks passed after Docker recovery. The user can create/sign into their own Clerk account at `/signup` or `/login`.

## Optional thinking support — 2026-09-23

Completed the additive [thinking-support batch](thinking-support.md) while retaining separate Ask AI/Try myself controls, existing home/navigation, historical events and session-level research definitions. No database tables or migrations were added.

- Full SQLite suite: **149 passed, 3 PostgreSQL-only skips**. After final context-label and per-turn first-request refinements, **25 focused PostgreSQL tests passed** across the new support, current-question context, chat upgrades and privacy.
- New checks cover rejection before charging when an attempt is missing; current-question matching; late attempts on an earlier question; initial/typed/exercise anchors; regeneration association; immutable verification revisions and replay conflicts; closed chats; cross-account/conversation isolation; deletion blocking; old metric preservation; raw export; invalidated anchors; and feedback purpose/language surviving provider fallback.
- QA and real production builds passed, including TypeScript validation. **17 Chrome scenarios passed**, including the two new thinking/verification cases and existing chat, progress, practice and privacy flows. Lost verification responses reuse their event ID, preserve the visible note and make no AI call. The mobile verification screenshot was visually inspected with no overflow.
- Separate real Clerk form smoke: **1 passed** on the final production build. API health confirmed database connected and Clerk mode. The updated website/API are running on localhost 3000/8000; temporary QA service was stopped.
- Tests used synthetic model responses and isolated test data. No paid provider call, real account creation or hosted deployment was performed. Model feedback is not certified grading, and live-model answer suppression is not guaranteed. Two existing Python dependency deprecation warnings remain.

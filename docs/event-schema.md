...
## Optional thinking support — question-turn-v1

This additive chat extension maps to constructs #1/#3 (attempt before assistance), #4/#7 (reported verification/evaluation) and #10 (optional assistance). Ask AI and Try myself remain distinct. No existing metric or guided protocol is redefined.

Question anchors are the session_started event, an explicit typed follow-up request, or a delivered related exercise. Each typed follow-up establishes a question turn, even when it continues the same subject; these are not inferred distinct semantic problems. Help buttons and regeneration keep the original anchor. New chat attempts and AI request/delivery events record server-validated `question_event_id` and `question_tracking_version: question-turn-v1`. Attempts may supply a valid same-conversation question anchor for offline/multiple-tab work; omission uses the current anchor. Historical events remain untouched; GET session returns a separately derived `question_tracking` view, marked as reconstructed from recorded turns, with per-turn activity and event references. Counts do not establish correctness, effort or independence outside this app.

`help_action: check_thinking` requests tier 3 with purpose `check_thinking`, requiring a saved attempt for the current question before allowance reservation. The backend supplies a dedicated feedback prompt to every fallback. It asks for a brief assessment and one next step without a worked solution, acknowledges uncertainty and avoids inventing mistakes. This is model feedback, not certified grading; answer suppression is not a semantic guarantee. It uses one explicit logical AI request, retains streaming/cancellation/replay machinery, and never changes the selected mode.

`answer_verification_reported` is an optional client event: `answer_event_id` UUID; `status: checked | found_issue | not_checked`; `method: calculation | example | source | reasoning | other | null`; `note` up to 2000 characters. Checked/issue reports require a method. Not-checked reports have no method. The target must be a delivered non-exercise response in the same owned chat. The server adds its question anchor and tracking version. Reports may be revised through new immutable events; latest report per answer is displayed. Exact ID retries are idempotent; conflicting reuse fails. Reports can be saved in completed chats, but pending account/conversation deletion blocks writes. They do not generate AI requests, create guided verification events, change old verification rates, or assert objective correctness. They are included in existing raw personal event export and removed with their conversation by existing erasure. Missing reports remain missing rather than being converted to not_checked.

The derived per-turn `attempt_before_first_request` is true only when an anchored attempt was saved before that turn's first recorded AI request, false when no such attempt preceded it, and null when no request exists. This uses server event order, not time spent thinking or offline authorship time. It does not replace session-level is_ai_first or infer use outside this app.
